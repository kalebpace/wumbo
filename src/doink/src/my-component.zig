...
export fn @"foo#concat"(leftPtr: [*]u8, leftLength: u32, rightPtr: [*]u8, rightLength: u32) [*]u32 {
    ...
    std.mem.writeIntLittle(u32, slice[0..4], @as(u32, @intCast(@intFromPtr(str))));
    std.mem.writeIntLittle(u32, slice[4..8], @as(u32, @intCast(leftLength + rightLength)));
    return @as([*]u32, @ptrCast(@alignCast(slice)));
}
...
export fn add(left: u32, right: u32) u32 {
    return left + right;
}
